...
class LRL(nn.Module):
      ...
      def add_noise_chw(self, x, prob = 0.2):
        b, c, d, h, w = x.shape
        ...
        random_num = torch.randn((c*d*h*w,),device='cuda')
        random_num[random_change.bool()]=0
        noise_x += random_num

        return noise_x.reshape(b,c,d,h,w)

      def forward(self, input, M=1, noise_branch = False, seg_only = False):
        x0_x = self.conv0_x(input)#B,C0,D,H,W
        x1_x = self.conv1_x(self.pool(x0_x))#B,C1,D,H/2,W/2
        x2_x = self.conv2_x(self.pool(x1_x))#B,C2,D,H/4,W/4
        x3_x = self.conv3_x(self.pool(x2_x))#B,C3,D,H/8,W/8
        x4_x = self.conv4_x(self.pool(x3_x))#B,C4,D,H/16,W/16
        x4_ = self.conv_down(x4_x)#B,C4,D,H/32=h,W/32=w

        # change to four dimensions    
        x4_d = torch.transpose(x4_, 1, 2).flatten(start_dim=2)#B,D,C4*h*w
 
        mean = self.mean_vec(x4_d)#B,D,C3//2
 
        B, depth, D = mean.size()
        identity = torch.eye(D).unsqueeze(0).expand(B * depth ,-1, -1).cuda() * 10.0
 
        covar_half_vec = self.covar_half(x4_d)#B,D,C3//2
        ...
        Z = self.reparameterize(mean,  covar_half, M)#B*M,D,C3//2

        Z_ = self.transform(Z).view(B * M, x4_.size()[-4], x4_.size()[-3], x4_.size()[-2], x4_.size()[-1])#B*M,C4,D,h,w
        ...
        x4_0 = torch.cat([x4_0_z, x4_0_v], 1)# x4_0_v, from another
        ...
        if seg_only:
           ...
           return output, output_noise, mean, covar.view(-1, depth, D, D), x_ori
        
        else:
          return output, mean, covar.view(-1, depth, D, D), x_ori


if __name__ == '__main__':
    lrl = LRL(input_channels=3,num_classes=10, resize_w = 160, resize_h = 160)
    img = torch.zeros((1,3,96, 160, 160))
    out = lrl(img)
