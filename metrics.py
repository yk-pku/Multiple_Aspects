...
def dice_coef_per_slice(output, target, numpy=False):
    smooth = 1e-8
    N = output.size(0)
    slice_num = output.size(2)

    output = output[:,1:,:,:,:]
    target = target[:,1:,:,:,:]

    if numpy:
      output = output.reshape(slice_num, -1)
      target = target.reshape(slice_num, -1)
    else:
      output = output.contiguous().view(slice_num, -1).detach().cpu().numpy()
      target = target.contiguous().view(slice_num, -1).detach().cpu().numpy()
